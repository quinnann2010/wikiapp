...
public class ExploreFragment extends Fragment {
    ...
    /* LOAD RANDOM ARTICLES FROM API https://api.wikimedia.org/w/api.php?action=query&list=random&rnlimit=5&format=json

    private void loadExploreContent() {
        // URL to get random articles
        String url = "https://api.wikimedia.org/w/api.php?action=query&list=random&rnlimit=8&format=json";
        ...
        client.newCall(request).enqueue(new Callback() { // Asynchronous call
            ...
            public void onResponse(@NonNull okhttp3.Call call, @NonNull Response response) throws IOException {
                if (!response.isSuccessful()) throw new IOException("Unexpected code " + response); // Check response
                ...
                // Retrieve the list of random articles
                if (jsonObject.has("query")) {
                    JsonObject queryObject = jsonObject.getAsJsonObject("query");
                    JsonArray randomArticles = queryObject.getAsJsonArray("random");
                    for (int i = 0; i < randomArticles.size(); i++) {
                        JsonObject article = randomArticles.get(i).getAsJsonObject();
                        String title = article.get("title").getAsString(); // Get article title
                        String articleUrl = "https://en.wikipedia.org/wiki/" + URLEncoder.encode(title, "UTF-8"); // Create URL for article

                        // Create a placeholder for the image URL
                        String imageUrl = ""; // Default to empty
                        if (article.has("thumbnail")) { // Check if thumbnail exists
                            JsonObject thumbnail = article.getAsJsonObject("thumbnail");
                            imageUrl = thumbnail.get("source").getAsString(); // Get image URL
                        }

                        articles.add(new Article(articleUrl, title, "", 0, imageUrl)); // Add article to the list with image URL
                    }
                }
